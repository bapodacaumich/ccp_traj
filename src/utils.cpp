...
TimeInterval compute_time_intervals(const std::vector<std::vector<float>>& knot_points,
                                    float velocity,
                                    size_t n_timesteps
                                    ) {
    ...
    for (size_t i = 0; i < knot_points.size()-1; i++)
    {
        float dx = knot_points[i+1][0] - knot_points[i][0];
        float dy = knot_points[i+1][1] - knot_points[i][1];
        float dz = knot_points[i+1][2] - knot_points[i][2];
        float d = sqrt(dx*dx + dy*dy + dz*dz);
        ds.push_back(d);
        total_length += d;
    }

    float total_time = total_length / velocity; // total path time
    ...
    for (size_t i = 0; i < ds.size(); i++) {
        ...
        num_dt_per_interval.push_back((int)(dt_knot[i] / dt));
        extra_dt.push_back(fmod((ds[i] / velocity), dt));
    }

    // construct dts
    TimeInterval time_intervals;
    time_intervals.knot_idx.push_back(0);

    for (size_t i = 0; i < num_dt_per_interval.size(); i++) {
        for (size_t j = 0; j < num_dt_per_interval[i]; j++) {
            time_intervals.dt.push_back(dt);
        }
        time_intervals.dt.push_back(extra_dt[i]);
        time_intervals.knot_idx.push_back(time_intervals.knot_idx.back() + num_dt_per_interval[i] + 1);
    }
